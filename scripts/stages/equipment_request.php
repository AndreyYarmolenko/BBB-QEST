<?php
	...
	$telegram = getBaseDataTelegram($idx, 41, $_REQUEST['RequestID'], $draft);
	...
	if ($draft == 0){
		...
		$VALUES['equipment_id'] = $tg_id;
		//$last_idx = em_insert('bb_equipment_approval', $VALUES);
		$VALUES['draft'] = 0;
		$VALUES['approved'] = 1;
		...
		$query = "SELECT `number` FROM bb_equipment WHERE `request_number` != ".$_REQUEST['RequestID'];
		$res_arr = array();
	...
	em_update('bb_equipment_request', $VALUES, $where);
	...
	$history = array('idx'=>$idx, 'task_type'=>41, 'status'=>$status, 'id_user'=>$id_user, 'old_data'=>$old_data);
	...
	$query = "UPDATE bb_equipment SET `name` = '$name', description = '$description', `number` = '$number', life_time = '$life_time', pending_design = '$pending_design', drawing2d = '$drawing_2d', drawing3d = '$drawing_3d', addImages = '$add_images', add_spec = '$add_spec', estimated_unit_price = $estimation_price, approved = $approved, alternative_id = $alternative/*, request_number = $request_number*/  WHERE id = $tg_id";

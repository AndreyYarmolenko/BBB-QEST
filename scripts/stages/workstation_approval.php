<?php
	...
	if($draft == 3) {
		...
		//$approved = 2;
		$query = "UPDATE bb_workstation_approval SET approved = $approved,  draft = 0 WHERE RequestID = $where";
	...
	//echo $draft;
	$query_approved = "UPDATE bb_workstation SET `approved` = $approved WHERE `id` = $tgId";
	if(!$db->query($query_approved)) echo '{"success":false,"message":"' . $db->errno . ' ' . $db->error . '"}';
	$db->commit();

	$query = "UPDATE bb_order_tasks SET `status` = 4 WHERE task_type = 44";
	if(!$db->query($query)) echo '{"success":false,"message":"' . $db->errno . ' ' . $db->error . '"}';
	$db->commit();

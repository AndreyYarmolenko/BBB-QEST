<?php
	...
	$VALUES['RequestedDate'] = transform_greenwich($_REQUEST['RequestedDate']);
	 ...
	 $telegram = getBaseDataTelegram($idx, 23, $RequestID, $draft);
	 $old_data = getOldData($table, $idx);

	$procurementArr =  json_decode(stripcslashes($procurement));
	$table_op = "";
	for($i=0; $i<count($procurementArr); $i++){
		$procurementArr[$i] = (array)$procurementArr[$i];
		switch($procurementArr[$i]['el_type']){
		...
		$query = "UPDATE ".$table_op."SET `received` = ".$procurementArr[$i]['received']." WHERE `id` = ".$procurementArr[$i]['row_id'];
		em_query($query);
	}

	if($draft == 1) {
	    $status = getStatusDraft();
	    em_update_order_task(23, $status, $reject=0);
	    $where = " `idx` = ".$idx."";
        ...
        if($count==0){
			...
			FROM `bb_procurement_request` WHERE `idx` =".$idx."";
...
$history = array('idx'=>$idx, 'task_type'=>23, 'status'=>$status, 'id_user'=>$id_user, 'old_data'=>$old_data);
setHistoryRecord($history);

<?php	
	...
	$telegram = getBaseDataTelegram($idx, 21, $_REQUEST['RequestID'], $draft);
	...
	if ($draft == 0){
		...
		$VALUES['tool_id'] = $tg_id;
		//$last_idx = em_insert('bb_tooling_approval', $VALUES);
		$VALUES['draft'] = 0;
		$VALUES['approved'] = 1;
		...
		$query = "SELECT `number` FROM bb_tool_gage WHERE `request_number` != ".$_REQUEST['RequestID'];
		$res_arr = array();
	...
	em_update('bb_tooling_request', $VALUES, $where);
	...
	$history = array('idx'=>$idx, 'task_type'=>21, 'status'=>$status, 'id_user'=>$id_user, 'old_data'=>$old_data);
	...
	$tool_gage_type = $_REQUEST['tool_gage_type'];
	//$request_number = $_REQUEST['RequestID'] ? $_REQUEST['RequestID'] : 0;
	$query = "UPDATE bb_tool_gage SET `name` = '$name', description = '$description', `number` = '$number', life_time = '$life_time', pending_design = '$pending_design', drawing2d = '$drawing_2d', drawing3d = '$drawing_3d', addImages = '$add_images', add_spec = '$add_spec', estimated_unit_price = $estimation_price, approved = $approved, tool_gage_type = $tool_gage_type, alternative_id = $alternative/*, request_number = $request_number*/ WHERE id = $tg_id";
	//echo $query;
